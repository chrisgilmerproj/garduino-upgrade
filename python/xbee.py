...
class xbee(object):
        ...
        START_IOPACKET   = '0x7e' #'0x6b'
        SERIES1_IOPACKET = '0x83'
       
        def find_packet(serial):
                print serial.read()
                print hex(ord(serial.read()))
                print xbee.START_IOPACKET
        ...
        def __str__(self):
                return "<xbee {app_id: %s, address_16: %s, rssi: %s, address_broadcast: %s, pan_broadcast: %s, total_samples: %s, digital: %s, analog: %s}>" \
                       % (self.app_id, self.address_16, self.rssi, self.address_broadcast, self.pan_broadcast, self.total_samples, self.digital_samples, self.analog_samples)
